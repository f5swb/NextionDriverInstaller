...
helpfiles () {
    ...
    wget "https://api.brandmeister.network/v1.0/groups/" -O /tmp/NextionDriver/groups.txt
    cp /tmp/NextionDriver/groups.txt $FILESDIR
    echo "+ Copying groups" and users files"
    wget "https://database.radioid.net/static/user.csv" -O /tmp/NextionDriver/stripped.csv
